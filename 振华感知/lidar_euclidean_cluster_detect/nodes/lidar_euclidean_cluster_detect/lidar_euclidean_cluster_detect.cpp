...
//使用指针：可以更灵活地管理对象，特别是在需要传递nullptr或管理对象生命周期时。
//使用引用：语法上更简洁，适用于确定对象始终存在且不需要动态管理的情况。
...
//ransac是找到内外点，所以还需要pcl的extract对输入的点云分割
...
if (in_cloud_ptr->isOrganized())
  ...
  // 设置法线估计的视点为最大值--在这个角度出去能看见多少点全都算进去，这样的话精度尽可能最高
  ...
  // 创建一个指向pcl::PointNormal类型的点云指针
  //pcl::PointNormal是一个结构体，它包含了点云中的一个点的坐标（x, y, z）和该点的法线向量（normal_x, normal_y, normal_z）。
  pcl::PointCloud<pcl::PointNormal>::Ptr diffnormals_cloud(new pcl::PointCloud<pcl::PointNormal>);
  // 将in_cloud_ptr中的点云数据复制到diffnormals_cloud中
  pcl::copyPointCloud<pcl::PointXYZ, pcl::PointNormal>(*in_cloud_ptr, *diffnormals_cloud);

  // Create DoN operator
  //两个pcl::PointNormal输出是因为其中一个存储法线差异
  ...
  range_cond->addComparison(pcl::FieldComparison<pcl::PointNormal>::ConstPtr(
    ...
    /**在PCL中，pcl::ComparisonOps是一个枚举类，定义了一系列的比较操作符，包括：

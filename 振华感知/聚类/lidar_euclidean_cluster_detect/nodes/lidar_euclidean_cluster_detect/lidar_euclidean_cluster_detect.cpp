...
void publishDetectedObjects(const autoware_msgs::CloudClusterArray &in_clusters)
{
  ...
  for (size_t i = 0; i < in_clusters.clusters.size(); i++)
  {
    ...
    detected_object.pose = in_clusters.clusters[i].bounding_box.pose;     // 不会�?到output frame, 仍为velodyne�?
    ...
    detected_object.convex_hull = in_clusters.clusters[i].convex_hull;    // 不会�?到output frame, 仍为velodyne�?
                          ...
                          const std::string &in_target_frame, const std_msgs::Header &in_header)
{
  if (in_target_frame != in_header.frame_id)
  {
    ...
    for (auto i = in_clusters.clusters.begin(); i != in_clusters.clusters.end(); i++)
    {
      ...
      try
      {
        _transform_listener->lookupTransform(in_target_frame, _velodyne_header.frame_id, ros::Time(),
                                             *_transform);
        pcl_ros::transformPointCloud(in_target_frame, *_transform, i->cloud, cluster_transformed.cloud);   // �?�?2: 在range_vision_fusion�?�?camera到指定坐标系的tf�?查一�?, 去除此限制后发现抖动很大
        ...
        if(_pose_estimation)
        {
          cluster_transformed.bounding_box.pose.orientation = i->bounding_box.pose.orientation;   // range_vision_fusion实际所需要的就是这里的boundingbox的pose, 但是这里没有对这�?位置作坐标变�?
        }
        else
        {
    ...
    publishDetectedObjects(clusters_transformed);   // ljc add: 如果要实现range vision fusion�?要求velodyne的输出以及lidar kf�?要求map下的输出, �?需要把这�?�的clusters_transformed改成in_clusters即可
                                                    // 但是考虑到后�?�?能的扩展�?, 还是在range vision fusion�?作改�?
  } else
  {
...
// Remove points up to a certain distance from the origin
...
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud)
{
  ...
  if (!_using_sensor_cloud)
  {
    ...
    if (_remove_ground)   // 输入�?来自ray_ground_filter的points_no_ground, 所以不需要再去除地面
    {
